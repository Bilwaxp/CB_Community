@echo off
setlocal enabledelayedexpansion
title CB_Community - Finalisation et Hebergement
color 0A
cls

cd /d "%~dp0"

echo.
echo ========================================
echo   CB_COMMUNITY - FINALISATION COMPLETE
echo ========================================
echo.
echo Ce script va:
echo   1. Corriger toutes les erreurs
echo   2. Preparer le projet
echo   3. Compiler le site
echo   4. Heberger le site
echo.
echo ========================================
echo.
pause

REM ============================================
REM ETAPE 1: Verification des prerequis
REM ============================================
echo [ETAPE 1/8] Verification des pre-requis...
echo.

REM Chercher Node.js
set "NODE_FOUND=0"
if exist "C:\Program Files\nodejs\node.exe" (
    set "PATH=%PATH%;C:\Program Files\nodejs"
    set "NODE_FOUND=1"
)
if exist "C:\Program Files (x86)\nodejs\node.exe" (
    set "PATH=%PATH%;C:\Program Files (x86)\nodejs"
    set "NODE_FOUND=1"
)
if exist "%LOCALAPPDATA%\Programs\nodejs\node.exe" (
    set "PATH=%PATH%;%LOCALAPPDATA%\Programs\nodejs"
    set "NODE_FOUND=1"
)

REM Tester Node.js
node --version >nul 2>&1
if errorlevel 1 (
    echo [ERREUR] Node.js n'est pas installe!
    echo Telechargez-le sur: https://nodejs.org
    pause
    exit /b 1
)
echo [OK] Node.js installe
node --version

REM Verifier npm
npm --version >nul 2>&1
if errorlevel 1 (
    echo [ERREUR] npm n'est pas installe!
    pause
    exit /b 1
)
echo [OK] npm installe
npm --version
echo.

REM ============================================
REM ETAPE 2: Arret des processus existants
REM ============================================
echo [ETAPE 2/8] Arret des processus existants...
taskkill /F /IM node.exe >nul 2>&1
timeout /t 2 /nobreak >nul
echo [OK] Processus arretes
echo.

REM ============================================
REM ETAPE 3: Configuration du fichier .env
REM ============================================
echo [ETAPE 3/8] Configuration du fichier .env...
if not exist ".env" (
    echo Creation du fichier .env...
    (
        echo # Base de donnees SQLite
        echo DATABASE_URL=file:./prisma/dev.db
        echo.
        echo # NextAuth Configuration
        echo NEXTAUTH_URL=http://localhost:3000
        echo NEXTAUTH_SECRET=cb-community-secret-key-changez-en-production
        echo.
        echo # Google OAuth ^(Optionnel^)
        echo GOOGLE_CLIENT_ID=
        echo GOOGLE_CLIENT_SECRET=
        echo.
        echo # Stripe Configuration ^(Optionnel^)
        echo STRIPE_SECRET_KEY=
        echo STRIPE_PUBLISHABLE_KEY=
        echo STRIPE_WEBHOOK_SECRET=
        echo STRIPE_PRO_PRICE_ID=
        echo STRIPE_VIP_PRICE_ID=
        echo.
        echo # Configuration Email SMTP
        echo SMTP_HOST=smtp.gmail.com
        echo SMTP_PORT=587
        echo SMTP_USER=cbcommunity7@gmail.com
        echo SMTP_PASSWORD=
        echo SMTP_FROM=cbcommunity7@gmail.com
        echo.
        echo # Environnement
        echo NODE_ENV=production
    ) > .env
    echo [OK] Fichier .env cree
) else (
    echo [OK] Fichier .env existe deja
    REM Verifier DATABASE_URL
    findstr /C:"DATABASE_URL" .env >nul 2>&1
    if errorlevel 1 (
        echo Ajout de DATABASE_URL...
        echo DATABASE_URL=file:./prisma/dev.db >> .env
    )
)
echo.

REM ============================================
REM ETAPE 4: Nettoyage
REM ============================================
echo [ETAPE 4/8] Nettoyage...
if exist ".next" (
    echo Suppression de l'ancien build...
    rmdir /s /q ".next" >nul 2>&1
)
if exist "node_modules\.cache" (
    rmdir /s /q "node_modules\.cache" >nul 2>&1
)
echo [OK] Nettoyage termine
echo.

REM ============================================
REM ETAPE 5: Installation des dependances
REM ============================================
echo [ETAPE 5/8] Installation des dependances...
if not exist "node_modules" (
    echo Installation en cours ^(cela peut prendre plusieurs minutes^)...
    call npm install --legacy-peer-deps
    if errorlevel 1 (
        echo [ERREUR] Echec de l'installation des dependances!
        pause
        exit /b 1
    )
    echo [OK] Dependances installees
) else (
    echo [OK] Dependances deja installees
    echo Mise a jour des dependances...
    call npm install --legacy-peer-deps
)
echo.

REM ============================================
REM ETAPE 6: Configuration Prisma et Base de donnees
REM ============================================
echo [ETAPE 6/8] Configuration Prisma et base de donnees...
call npx prisma generate
if errorlevel 1 (
    echo [ERREUR] Echec de la generation Prisma!
    pause
    exit /b 1
)
echo [OK] Client Prisma genere

REM Verifier si la base de donnees existe
if not exist "prisma\dev.db" (
    echo Creation de la base de donnees...
    call npx prisma db push --accept-data-loss
    if errorlevel 1 (
        echo [ERREUR] Echec de la creation de la base de donnees!
        pause
        exit /b 1
    )
    echo [OK] Base de donnees creee
) else (
    echo [OK] Base de donnees existe deja
    echo Mise a jour du schema...
    call npx prisma db push --accept-data-loss
)
echo.

REM ============================================
REM ETAPE 7: Build de production
REM ============================================
echo [ETAPE 7/8] Build de production...
echo.
echo Compilation en cours ^(cela peut prendre plusieurs minutes^)...
echo.
call npm run build
if errorlevel 1 (
    echo.
    echo [ERREUR] Echec du build!
    echo.
    echo Verifiez les erreurs ci-dessus et corrigez-les.
    echo.
    pause
    exit /b 1
)
echo.
echo [OK] Build de production termine avec succes!
echo.

REM ============================================
REM ETAPE 8: Trouver l'IP et afficher les informations
REM ============================================
echo [ETAPE 8/8] Preparation du demarrage...
echo.

REM Trouver l'IP locale
for /f "tokens=2 delims=:" %%a in ('ipconfig ^| findstr /i "IPv4"') do (
    set LOCAL_IP=%%a
    set LOCAL_IP=!LOCAL_IP:~1!
    goto :found_ip
)
:found_ip

cls
echo.
echo ========================================
echo   FINALISATION TERMINEE AVEC SUCCES!
echo ========================================
echo.
echo Le site est maintenant pret a etre heberge!
echo.
echo Informations importantes:
echo   - Base de donnees: prisma\dev.db
echo   - Port: 3000
echo   - Mode: Production
echo   - Build: .next\standalone
echo.
if defined LOCAL_IP (
    echo Votre IP locale: %LOCAL_IP%
    echo.
    echo Le site sera accessible sur:
    echo   - Local: http://localhost:3000
    echo   - Reseau: http://%LOCAL_IP%:3000
    echo.
    echo IMPORTANT: Pour l'acces depuis Internet:
    echo   1. Configurez le firewall ^(CONFIGURER_FIREWALL.bat^)
    echo   2. Configurez le port forwarding sur votre routeur
    echo   3. Changez NEXTAUTH_URL dans .env avec votre IP publique
    echo.
) else (
    echo Executez TROUVER_IP.bat pour trouver votre IP
    echo.
)
echo ========================================
echo.
echo Appuyez sur une touche pour demarrer le serveur...
pause >nul

REM ============================================
REM DEMARRAGE DU SERVEUR
REM ============================================
cls
echo.
echo ========================================
echo   SERVEUR EN COURS D'EXECUTION
echo ========================================
echo.
echo Le site est maintenant accessible!
echo.
if defined LOCAL_IP (
    echo   - Local: http://localhost:3000
    echo   - Reseau: http://%LOCAL_IP%:3000
) else (
    echo   - Local: http://localhost:3000
)
echo.
echo (Fermez cette fenetre pour arreter le serveur)
echo.
echo ========================================
echo.

REM Demarrer le serveur en mode standalone si disponible
if exist ".next\standalone\server.js" (
    echo [INFO] Mode standalone detecte
    echo Demarrage avec node .next/standalone/server.js
    echo.
    node .next/standalone/server.js
) else (
    echo [INFO] Mode normal
    echo Demarrage avec npm start
    echo.
    call npm start
)

pause
